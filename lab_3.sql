...
--on this you must give EmployeeID, DOB, Gender & Hiredate.
	 create or alter procedure pr_Empl_gate
		@fname varchar(50),
		@lname varchar(50)
...
--4. Create a procedure that will accepts any integer and if salary is between provided integer, then those
	--employee list comes in output.
	  ...
	  CREATE OR ALTER PROCEDURE PR_EMPL_SAL
		   @MINSA INT ,
			@MAX INT
	   ...
	   BEGIN
			 SELECT * FROM Employee
			 WHERE Salary BETWEEN @MINSA AND @MAX
		END
...
--served.
      CREATE OR ALTER PROCEDURE PR_EMPLOYE_GENDER
	  @GNAME VARCHAR
	  AS
	  BEGIN
		SELECT * FROM
		Employee
		WHERE Gender LIKE @GNAME+'%'
	END
...
--data will come.
   ...
   CREATE OR ALTER PROCEDURE PR_DEPT_DATA
    @FNAME VARCHAR(50),
	@dNAME VARCHAR(50)
AS 
BEGIN
...
--get all the departments with all data.
 ...
 create or alter procedure pr_DEPT_HISDATA
	@LOC VARCHAR(50)
AS 
BEGIN
   SELECT *
...
--related data.
	...
	BEGIN
		 SELECT *
		   ...
		   JOIN Projects
		   ON Projects.DepartmentID=Departments.DepartmentID
		   WHERE Projects.StartDate=@FDATE and Projects.EndDate=@ENDDATE
	end
...
--provide all data with Department Name, Manager Name with Project Name & Starting Ending Dates. 
	...
	AS 
	BEGIN
		 SELECT Departments.DepartmentName,Employee.FirstName + Employee.LastName ,Projects.ProjectName,Projects.StartDate,Projects.EndDate
			   FROM Departments
			   JOIN Employee
			   ON Departments.DepartmentID=Employee.DepartmentID
			   JOIN Projects
		...
		PR_PROJECT_DATA 'Project Alpha','New York'
